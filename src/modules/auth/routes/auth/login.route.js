...
const { loginDtoSchema } = dto;
const { AuthConstants } = constants;

export function loginRoute(router, controller) {
  router.post('/login', validateDto(loginDtoSchema, AuthConstants.ERRORS.LOGIN_FAILED), async (req, res) => {
    try {
